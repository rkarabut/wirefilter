...
pub struct TwoWaySearcher {
    ...
    // We need this because `memmem::TwoWaySearcher` wants a lifetime for the data it refers to,
    // but we don't want to tie it to the lifetime of `TwoWaySearcher`, since our data is
    // heap-allocated and is guaranteed to deref to the same address across moves of the
    // container. Hence, we use `static` as a substitute lifetime and it points to the same the
    // data as `needle`.
